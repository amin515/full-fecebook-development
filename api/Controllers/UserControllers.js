...
import { hashPassword, pwordVerify } from "../utility/hash.js";
import { createToken, verifyToken } from "../utility/jsonWebToken.js";
import { getRandom } from "../utility/math.js";
import { passwordResetLink, sentActivationLink } from "../utility/sendMail.js";
    ...
    if (user) {
      ...
      if (emailData) {
        ...
        sentActivationLink(user.email, {
          ...
          }/api/v1/user/activation/${activationToken}`,
          code: accessCode,
          mail: user.email,
        });
      ...
      if (mobileData) {
        ...
        sendOTP(
          user.cell,
          `Hi, ${user.first_name} ${user.sur_name} Your OTP code is ${accessCode}`
        );

        res
          .status(200)
          .cookie("otp", user.cell, {
            expires: new Date(Date.now() + 1000 * 60 * 15),
          })
          .json({
            message: "User create successful",
...
export const resendActivationLink = async (req, res, next) => {
  ...
  try {
    ...
    if (mobileData) {
      ...
      sendOTP(
        checkMobile.cell,
        `Hi, ${checkMobile.first_name} ${checkMobile.sur_name} Your OTP code is ${accessCode}`
      );
